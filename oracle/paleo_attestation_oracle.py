#!/usr/bin/env python3
"""paleo-attestation-oracle -- a deterministic attestation-integrity decider for the `paleo` knowledge domain.

An **oracle** *deterministically decides* the truth of a case. This one decides, for a `paleo` knowledge
capsule, whether it is a well-formed, provenance-carrying attestation. VALID iff: domain=='paleo', a non-empty
...
def decide(c):
    if c.get("domain") != "paleo": return "INVALID"
...
def _run_probes(path):
    ...
    print("paleo attestation oracle: probes=%d | recall=%.3f | false_positives=%d | verdict=%s"%(len(probes),recall,fp,"PASS" if ok else "FAIL"))
